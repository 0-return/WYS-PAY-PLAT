        ...
        <div class="layui-card-body" style="padding: 15px;">
            <div class="layui-form" lay-filter="component-form-group">
                <div class="layui-form-item school">
                    ...
                    <div class="layui-input-block agent">
...
<script>
    ...
    layui.config({
        ...
        if(type==1){
            $('.agent .layui-select-title input').val('店长')
        }else{
            $('.agent .layui-select-title input').val('收银员')
        }
        $('.agentid').val(type);
        ...
        $.post("{{url('/api/merchant/merchant_info')}}",
        {
            token:token,
            ...
            $('.password').val(res.data.name);
        });
        
        $('.submit').on('click', function(){
            if($('.password').val()==''){
                $.post("{{url('/api/merchant/up_merchant')}}",
                {
                    token:token,
                    store_id:store_id,
                    name:$('.name').val(),
