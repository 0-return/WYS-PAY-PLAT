            ...
            <div class="layui-col-md12">
              <div class="layui-card"> 
                ...
                <div class="layui-card-body">
                  ...
                  <!-- 判断状态 -->
                  <script type="text/html" id="mchnt">
                    @{{#  if(d.trans_zero_flag == 1){ }}
                      <span class="cur">开通</span>
                    @{{#  } else { }}
                      未开通
                    @{{#  } }}
                  </script>
    ...
    <div class="layui-form">
      <div class="layui-form-item">
        ...
        <div class="layui-form" lay-filter="component-form-group" style="width:100%;display: inline-block;">
          ...
          <div class="layui-form-item">
            <label class="layui-form-label" style="width:90px;text-align: left;">可提现余额:</label>
          ...
          <div class="layui-form-item">
            ...
            <div class="layui-input-block item5">
              <input type="number" placeholder="请输入需要提现的金额" class="layui-input" id="jine" style="width:60%;float:left;">
...
<input type="hidden" class="js_nl_mercId">
  ...
  <script src="{{asset('/layuiadmin/layui/jquery.qrcode.min.js')}}"></script>
    ...
    layui.config({
      ...
      table.render({
        ...
        ,cols: [[
          ...
          ,{field:'store_id', title: '是否开通d0',templet:'#mchnt'}
          ,{field:'mchnt_cd', title: '商户号'}   
          ,{width:160,align:'center', fixed: 'right', toolbar: '#table-content-list',title: '操作'}       
        ]]
        ,response: {
      ...
      table.on('tool(test-table-page)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'open'){ //开启
          ...
          $.post("{{url('/api/fuiou/select_money')}}",
          {
              token:token,
              store_id:e.store_id,
              mchnt_cd:e.mchnt_cd
              ...
              console.log(res);
              if(res.status==1){
                $('.item2').html(res.data.not_settle_amt)
      ...
      $('.submit').click(function(){
        if($('#jine').val()>$('.item2').html() || $('#jine').val() == 0){
          ...
          var amt=$('#jine').val()
          $.post("{{url('/api/fuiou/out_money')}}",
          {
              token:token,
              store_id:$('.js_storeid').val(),
              mchnt_cd:$('.js_nl_mercId').val(),
              amt:amt
          },function(res){
              console.log(res);
              if(res.status==1){
    ...
    $('#Upload').click(function(){
      ...
      $.ajax({   
        url: "{{url('/api/fuiou/import_mercId?token=')}}"+token+"&store_id="+store_id,  
        type: 'POST',  
        data: formData,  
        async: false,  
        cache: false,  
    ...
    var active = {
      ...
      batchdel: function(){
        ...
        for(var i=0;i<checkData.length;i++){ 
          arrs.push(checkData[i].mchnt_cd);
        }
        var mchnt_cd=arrs.join()
    ...
    form.on('submit(LAY-app-contlist-search)', function(data){
      var obj = data.field
      console.log(obj)
      var mchnt_cd = data.field.schoolname;    
      console.log(data);
      //执行重载
