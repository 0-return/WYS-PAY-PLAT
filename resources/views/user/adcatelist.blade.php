              ...
              <div class="layui-card"> 
                ...
                <div class="layui-card-body">
                  <div class="layui-btn-container" style="font-size:14px;">
                    <a class="layui-btn layui-btn-primary" lay-href="{{url('/user/addadcate')}}" style="display: block;width: 122px;">添加分类</a>
    ...
    layui.config({
      base: '../../layuiadmin/' //静态资源所在路径
        ...
        table.render({
            ...
            ,url: "{{url('/api/adcate/adcate_lists')}}"
            ,method: 'post'
            ,where:{
              token:token,              
            }
         ...
         table.on('tool(test-table-page)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
          ...
          if(layEvent === 'del'){
            layer.confirm('确认删除此消息?',{icon: 2}, function(index){
              $.post("{{url('/api/adcate/adcate_del')}}",
              {
                 token:token,id:e.id
            ...
            $('.edit').attr('lay-href',"{{url('/user/editadcate?')}}"+e.id);
          }
