                ...
                <div class="layui-card-body">
                  ...
                  <script type="text/html" id="table-content-list" class="layui-btn-small">                    
                    ...
                    {{#  if(d.ways_type == 8005 || d.ways_type == 6005){ }}
                      <a class="layui-btn layui-btn-danger layui-btn-xs shua" lay-event="shua">修改成本费率</a>
                    {{#  } else if(d.ways_type == 8004 || d.ways_type == 6004){ }}
                      <a class="layui-btn layui-btn-danger layui-btn-xs sao" lay-event="sao">修改成本费率</a>
                    {{#  } else { }}
                      <a class="layui-btn layui-btn-danger layui-btn-xs edit" lay-event="edit">修改成本费率</a>
                    {{#  } }}
                  </script>
                </div>
              </div>
    ...
    layui.config({
      ...
      $('.layui-card-header').html(agentName);
        ...
        table.render({
            ...
            ,url: "<?php echo e(url('/api/user/user_ways_all')); ?>"
            ,method: 'post'
            ,where:{
              token:token,   
              user_id:user_id           
            ...
            ,cols: [[
              ...
              ,{field:'rate', title: '成本费率',templet:'#rate'}
        ...
        $('.submit').click(function(){
          $.post("<?php echo e(url('/api/user/edit_user_rate')); ?>",
          {
              token:token,
              user_id:user_id,
              ways_type:$('.type').val(),
        ...
        $('.submits').click(function(){
          $.post("<?php echo e(url('/api/user/edit_user_un_rate')); ?>",
          {
              token:token,
              user_id:user_id,
              ways_type:$('.types').val(),
        ...
        $('.submitsao').click(function(){
          $.post("<?php echo e(url('/api/user/edit_user_unqr_rate')); ?>",
          {
              token:token,
              user_id:user_id,
              ways_type:$('.types').val(),
