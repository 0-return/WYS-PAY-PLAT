        ...
        <div class="layui-card-body" style="padding: 15px;">
            <div class="layui-form" lay-filter="component-form-group">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block" style="width: 74%;float: left;display: inline-block;margin-left: 0px;">
...
<script>
    ...
    $.post("<?php echo e(url('/api/user/weixin_config')); ?>",
    {
        token:token,
        type:'2'
        ...
        $('.layui-form .layui-form-item').eq(1).find('input').val(res.data.app_secret);
        $('.layui-form .layui-form-item').eq(2).find('input').val(res.data.wx_merchant_id);
        $('.layui-form .layui-form-item').eq(3).find('input').val(res.data.key);
        $('.layui-form .layui-form-item').eq(4).find('input.zs').val(res.data.cert_path);
        $('.layui-form .layui-form-item').eq(5).find('input.my').val(res.data.key_path);
        $('.layui-form .layui-form-item').eq(6).find('input.ym').val(res.data.auth_path);
    },"json");
    ...
    // demo3
    upload.render({
        url : "<?php echo e(url('/api/basequery/webupload?act=file')); ?>"+'&token='+token+'&type='+'wxauth',  //提交到的地址 可以自定义其他参数
    ...
    $('.submit').on('click', function(){
        $.post("<?php echo e(url('/api/user/weixin_config')); ?>",
        {
            ...
            app_secret:$('.layui-form .layui-form-item').eq(1).find('input').val(),
            wx_merchant_id:$('.layui-form .layui-form-item').eq(2).find('input').val(),
            key:$('.layui-form .layui-form-item').eq(3).find('input').val(),
            
            cert_path:$('.layui-form .layui-form-item').eq(4).find('input').val(),
            key_path:$('.layui-form .layui-form-item').eq(5).find('input').val(),
            auth_path:$('.layui-form .layui-form-item').eq(6).find('input').val(),
