            ...
            <div class="layui-form" lay-filter="component-form-group"> 
                ...
                <div class="layui-form-item class">
                    ...
                    <div class="layui-input-block" style="width:40%;display: inline-block;margin-left:0">
                        <select name="range" id="range" xm-select="range">
...
<script>
    ...
    var ad_p_id = localStorage.getItem("ad_p_id");
    var user_ids = localStorage.getItem("user_ids");
    var store_key_ids = localStorage.getItem("store_key_ids");
...
//单条信息------------------------------------------------------------------------
        $.post("<?php echo e(url('/api/ad/ad_info')); ?>",
        {
            token:token,
            id:id,            
            ...
            $('.start-item').val(res.data.s_time);
            $('.end-item').val(res.data.e_time);
            $('.con').html(res.data.copy_content);
            $('.user_id').val(res.data.user_ids);

            $('.position_id').val(res.data.ad_p_id);
...
//------------------------------------------------------------------------
        ...
        formSelects.config('position', {
            ...
            success: function(id, url, searchVal, result){      //使用远程方式的success回调
                var stuclass=ad_p_id.split(',');//转换数组
                formSelects.value('position', stuclass);
                console.log(stuclass);
               

            }
        }).data('position', 'server', {
            url:"<?php echo e(url('/api/ad/ad_p_id?token=')); ?>"+token
        });
        ...
        formSelects.config('range', {
            beforeSuccess: function(id, url, searchVal, result){
                ...
                for(var i=0;i<result.length;i++){
                    ...
                    var data ={"value":result[i].id,"name":result[i].name}
                    arrr.push(data);
                }
                console.log(arrr);
                //然后返回数据
            ...
            success: function(id, url, searchVal, result){      //使用远程方式的success回调
                var stuclass=user_ids.split(',');//转换数组
                formSelects.value('range', stuclass);
                console.log(stuclass);
            }
        }).data('range', 'server', {
            url:"<?php echo e(url('/api/user/get_sub_users?token=')); ?>"+token
        });
            ...
            if(aa.length>1){
                ...
                if(b==a && isAdd==false){
                    ...
                    formSelects.config('store', {
                        ...
                        url:"<?php echo e(url('/api/user/store_lists?token=')); ?>"+token+"&user_id="+$('.user_id').val()
                    });
                } 
            }
        ...
        formSelects.config('store', {
            ...
            url:"<?php echo e(url('/api/user/store_lists?token=')); ?>"+token+"&user_id="+user_ids
        }); 
       

        //多图片上传
        upload.render({            
            url : "<?php echo e(url('/api/basequery/webupload?act=images')); ?>"+'&token='+token
            ,elem: '.test1'
            ,method : 'POST'
            ,type : 'images'
            ,ext : 'jpg|png|gif'
        ...
        $('.submit').on('click', function(){
            ...
            $.post("<?php echo e(url('/api/ad/ad_up')); ?>",
            {
                token:token,
                id:id,
                title:$('.title').val(),
