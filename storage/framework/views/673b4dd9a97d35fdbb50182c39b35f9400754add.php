  ...
  <script src="<?php echo e(asset('/layuiadmin/layui/jquery.qrcode.min.js')); ?>"></script>
    ...
    layui.config({
      ...
      form.on('select(payitem)', function(data){
        ...
        $.post("<?php echo e(url('/api/newland/get_da_info')); ?>",
          {
              token:token,
      ...
      table.render({
        ...
        ,url: "<?php echo e(url('/api/mybank/store_list')); ?>"
        ,method: 'post'
        ,where:{
          token:token,  
          store_id:store_id            
        ...
        ,cols: [[
          ...
          ,{field:'OutMerchantId', title: '平台ID'}
          ,{field:'MerchantName', title: '门店名称'}
      ...
      table.on('tool(test-table-page)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'open'){ //开启
          $.post("<?php echo e(url('/api/newland/open_da')); ?>",
          {
              token:token,
              store_id:e.store_id,
          ...
          $.ajax({
            url : "<?php echo e(url('/api/newland/store_list')); ?>",
          ...
          $('.withdrawrecord').attr('lay-href',"<?php echo e(url('/user/withdrawrecord?store_id=')); ?>"+e.store_id+"&nl_mercId="+e.nl_mercId);
        }else if(layEvent === 'tixianrate'){//提现费率
          $('.js_store_id').val(e.store_id)
          $('.js_nl_mercId').val(e.nl_mercId)
          $.post("<?php echo e(url('/api/newland/set_da_rate')); ?>",
          {
              token:token,
              store_id:e.store_id,
              nl_mercId:e.nl_mercId,
      ...
      $('.submit').click(function(){
        if($('#jine').val()>$('.item2').html() || $('#jine').val() == 0){
          ...
          $.post("<?php echo e(url('/api/newland/get_da')); ?>",
          {
              token:token,
              store_id:$('.js_storeid').val(),
    ...
    $('#Upload').click(function(){
      ...
      $.ajax({   
        url: "<?php echo e(url('/api/mybank/import_mercId?token=')); ?>"+token+"&store_id="+store_id,  
        type: 'POST',  
        data: formData,  
        async: false,  
        cache: false,  
    ...
    var active = {
      ...
      batchdel: function(){
        ...
        for(var i=0;i<checkData.length;i++){ 
          arrs.push(checkData[i].MerchantId);
        }
        var MerchantId=arrs.join()
        ...
        layer.confirm('确定删除吗？', function(index) {   
          ...
          $.post("<?php echo e(url('/api/mybank/del_store')); ?>",
          {
              token:token,
              store_id:store_id,
              MerchantId:arrs.join()               
