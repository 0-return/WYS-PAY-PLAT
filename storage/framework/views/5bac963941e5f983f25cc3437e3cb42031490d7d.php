    ...
    layui.config({
      base: '../../layuiadmin/' //静态资源所在路径
        ...
        table.render({
            ...
            ,url: "<?php echo e(url('/api/user/QrLists')); ?>"
            ,method: 'post'
            ,where:{
              token:token,              
            }
         ...
         table.on('tool(test-table-page)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
          ...
          if(layEvent === 'down'){
            $.post("<?php echo e(url('/api/user/DownloadQr')); ?>",
            {
                token:token,
                cno:e.cno
            },
            ...
            $('.bound').attr('lay-href',"<?php echo e(url('/user/bound?')); ?>"+e.cno)
          }else if(layEvent === 'unbound'){
            $('.unbound').attr('lay-href',"<?php echo e(url('/user/unbound?')); ?>"+e.cno)
          }
        ...
        $('.allcode').click(function(){
          $(this).attr('lay-href',"<?php echo e(url('/user/unbound')); ?>")
        })


        $('.submit').click(function(){          
          var index = layer.load(2);
          $.post("<?php echo e(url('/api/user/createQr')); ?>",
            {
                token:token,
                num:$('.num').val()
            },
