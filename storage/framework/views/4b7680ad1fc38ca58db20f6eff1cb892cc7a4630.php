  ...
  <script src="<?php echo e(asset('/layuiadmin/layui/layui.js')); ?>"></script> 
    ...
    layui.config({
      ...
      table.on('tool(test-table-page)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'edit'){ //审核
          $('.edit').attr('lay-href',"<?php echo e(url('/mb/editcashier?store_id=')); ?>"+store_id+"&merchant_id="+e.merchant_id+"&type="+e.type);
        }else if(layEvent === 'del'){
          
          layer.confirm('确认删除此收银员?',{icon: 2}, function(index){
            $.post("<?php echo e(url('/api/merchant/del_merchant')); ?>",
            {
      ...
      $('.addcashier').click(function(){
        $(this).attr('lay-href',"<?php echo e(url('/mb/addcashier?')); ?>"+store_id);
      })
