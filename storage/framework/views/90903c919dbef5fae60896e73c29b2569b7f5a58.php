      ...
      <div class="layui-card-body layui-row layui-col-space10">
        <div class="layui-form">
            ...
            <div class="layui-form-item">
                <label class="layui-form-label">阿里云app_key</label>
                <div class="layui-input-block">
                    <input type="text" name="schoolname" lay-verify="schoolname" autocomplete="off" placeholder="请输入阿里云app_key" class="layui-input templatename">
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label">阿里云app_secret</label>
                <div class="layui-input-block">
                    <input type="text" name="schoolname" lay-verify="schoolname" autocomplete="off" placeholder="请输入阿里云app_secret" class="layui-input templatedesc">
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label">短信签名SignName</label>
                <div class="layui-input-block">
                    <input type="text" name="schoolname" lay-verify="schoolname" autocomplete="off" placeholder="请输入短信签名SignName" class="layui-input templatedesc">
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label">短信模版TemplateCode</label>
                <div class="layui-input-block">
                    <input type="text" name="schoolname" lay-verify="schoolname" autocomplete="off" placeholder="请输入短信模版TemplateCode" class="layui-input templatedesc">
                </div>
            </div>
...
<script>
    ...
    layui.config({
        ...
        $.post("<?php echo e(url('/api/basequery/sms_type')); ?>",
        {
            token:token
        },function(data){
            console.log(data);
            $('.schooltypeid').val(data.data[0].type); 
            $('.schooltypename').val(data.data[0].type_desc);
            var optionStr = "";
            for(var i=0;i<data.data.length;i++){
                optionStr += "<option value='" + data.data[i].type + "'>"
                    + data.data[i].type_desc + "</option>";
            }    
            $("#schooltype").append(optionStr);
            layui.form.render('select');

            $.post("<?php echo e(url('/api/basequery/sms_info')); ?>",
            {
                token:token,
                type:data.data[0].type
            },function(res){
                console.log(res);
                $('.layui-form .layui-form-item').eq(1).find('input').val(res.data.app_key);
                $('.layui-form .layui-form-item').eq(2).find('input').val(res.data.app_secret);
                $('.layui-form .layui-form-item').eq(3).find('input').val(res.data.SignName);
                $('.layui-form .layui-form-item').eq(4).find('input').val(res.data.TemplateCode);
            },"json");
        ...
        $('.submit').on('click', function(){
            ...
            $.post("<?php echo e(url('/api/basequery/sms_info')); ?>",
            {
                ...
                app_key:$('.layui-form .layui-form-item').eq(1).find('input').val(),
                app_secret:$('.layui-form .layui-form-item').eq(2).find('input').val(),
                SignName:$('.layui-form .layui-form-item').eq(3).find('input').val(),
                TemplateCode:$('.layui-form .layui-form-item').eq(4).find('input').val()
