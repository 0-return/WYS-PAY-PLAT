              ...
              <div class="layui-card"> 
                ...
                <div class="layui-card-body">
                  <div class="layui-btn-container" style="font-size:14px;">
                    <a class="layui-btn layui-btn-primary" lay-href="<?php echo e(url('/user/addadcate')); ?>" style="display: block;width: 122px;">添加分类</a>
    ...
    layui.config({
      base: '../../layuiadmin/' //静态资源所在路径
        ...
        table.render({
            ...
            ,url: "<?php echo e(url('/api/adcate/adcate_lists')); ?>"
            ,method: 'post'
            ,where:{
              token:token,              
            }
         ...
         table.on('tool(test-table-page)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
          ...
          if(layEvent === 'del'){
            layer.confirm('确认删除此消息?',{icon: 2}, function(index){
              $.post("<?php echo e(url('/api/adcate/adcate_del')); ?>",
              {
                 token:token,id:e.id
            ...
            layer.open({
              ...
              maxmin: true,
              area: ['90%', '100%'],
              content: "<?php echo e(url('/user/adsee?')); ?>"+e.id
            });
          }else if(layEvent === 'edit'){
            //console.log(e.id);
            localStorage.setItem('id', e.id);
            localStorage.setItem('title', e.title);
            $('.edit').attr('lay-href',"<?php echo e(url('/user/editadcate?')); ?>"+e.id);
          }
