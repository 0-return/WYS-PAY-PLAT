...
<div class="register-con">
    ...
    <div class="input-con">
        <label>激活码</label>
        <input type="text" placeholder="请输入推广员ID(必填)" class="active-code" value="<?php echo e($_GET['s_code']); ?>" disabled="disabled" />
    </div>
...
<input type="hidden" class="login_type" value="<?php echo e($_GET['register_type']); ?>">
</body>
...
<script type="text/javascript" src="<?php echo e(asset('/school/js/jsencrypt.min.js')); ?>"></script>
<script>$(function() {FastClick.attach(document.body);});</script>
<script type="text/javascript">
    $('.login').click(function(){
        window.location.href = "<?php echo e(url('/school/login?login_type=')); ?>"+$('.login_type').val()+"&s_code="+$('.active-code').val();
    });
    //发送验证码
    ...
    function sendMessage() {
        ...
        InterValObj = window.setInterval(SetRemainTime, 1000); //启动计时器，1秒执行一次
        //向后台发送处理数据
        $.post("<?php echo e(url('/api/Sms/send')); ?>",
            {
                sign:data
            },
            function (res){
    ...
    $('.confirm').click(function(){
        ...
        if(pass==passagain){
            $.get("<?php echo e(url('/api/merchant/register')); ?>",
            {                
                phone:$('.js-tel').val(),
                password:$(".js-pass").val(),
                password_confirmed:$(".js-pass-again").val(),                
                msn_code:$('.code').val(),
                register_type:"<?php echo e($_GET['register_type']); ?>",                
                s_code:"<?php echo e($_GET['s_code']); ?>"
            },function(res){
                console.log(res);
                if(res.status==1){     
