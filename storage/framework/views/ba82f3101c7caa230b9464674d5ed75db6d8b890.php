            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body">
                    <div class="cmdlist-container">            
                        <img style='margin-top:-60px;' src="<?php echo e(asset('/school/images/touxiang.png')); ?>">            
                        <div class="xinxi" style="display: inline-block;">
                            <p class="name"></p>
                            <p class="phone"></p>
                            <p class="jhm"><a lay-href="<?php echo e(url('/user/qrcode')); ?>">激活码</a>:<span></span></p>
            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body">
                    <div class="cmdlist-container box"> 
                        <a class="layui-btn layui-btn-primary" lay-href="<?php echo e(url('/user/addagent')); ?>" id="agent" data-id="">添加代理商</a>
    ...
    layui.config({
        ...
        $("#role_fen").click(function(){
            ...
            layer.open({
              ...
              maxmin: true,
              area: ['70%', '70%'],
              content: "<?php echo e(url('/user/rolelist?')); ?>"+customer_id
            });
        });
        $("#rate").click(function(){
            localStorage.setItem('agentName', $('.name').html());
            var user_id=$(this).attr('data-id');
            $('.rate').attr('lay-href',"<?php echo e(url('/user/ratelist?')); ?>"+user_id);
        })
        $("#storerate").click(function(){
            localStorage.setItem('agentName', $('.name').html());
            var user_id=$(this).attr('data-id');
            $('.storerate').attr('lay-href',"<?php echo e(url('/user/storeratelist?')); ?>"+user_id);
        })
        $("#reward").click(function(){
            localStorage.setItem('agentName', $('.name').html());
            var user_id=$(this).attr('data-id');
            $('.reward').attr('lay-href',"<?php echo e(url('/user/reward?')); ?>"+user_id);
        })
        $("#putforward").click(function(){
            localStorage.setItem('agentName', $('.name').html());
            var user_id=$(this).attr('data-id');
            $('.putforward').attr('lay-href',"<?php echo e(url('/user/putforward?')); ?>"+user_id);
        })
        $("#storeconfig").click(function(){
            localStorage.setItem('agentName', $('.name').html());
            var user_id=$(this).attr('data-id');
            $('.storeconfig').attr('lay-href',"<?php echo e(url('/user/storeconfig?')); ?>"+user_id);
        })
        $("#chaxun").click(function(){
            localStorage.setItem('agentName', $('.name').html());
            var user_id=$(this).attr('data-id');
            var user_name=$('.name').html();
            $('.chaxun').attr('lay-href',"<?php echo e(url('/user/reconciliation?user_id=')); ?>"+user_id+"&user_name="+user_name);
        })
        $("#settle").click(function(){
            localStorage.setItem('agentName', $('.name').html());
            var user_id=$(this).attr('data-id');
            $('.settle').attr('lay-href',"<?php echo e(url('/user/settledetail?user_id=')); ?>"+user_id);
        })
