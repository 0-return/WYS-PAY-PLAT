    ...
    <div class="layui-form">
      <div class="layui-form-item box">
        <div style="padding-bottom:14px;">
          ...
          <a class="layui-btn layui-btn-normal layui-btn-xs device" lay-event="device" lay-href="<?php echo e(url('/user/devicelist')); ?>">设备管理</a>
          
          <a class="layui-btn layui-btn-danger layui-btn-xs edit" lay-event="edit" lay-href="<?php echo e(url('/user/editstore')); ?>">门店修改</a>
  ...
  <script src="<?php echo e(asset('/layuiadmin/layui/jquery.qrcode.min.js')); ?>"></script>
    ...
    layui.config({
      ...
      var $ = layui.$
        ...
        // 地区选择
        $.ajax({
          url : "<?php echo e(url('/api/basequery/city')); ?>",
          data : {area_code:'1'},
          type : 'get',
          success : function(data) {
              console.log(data);
        ...
        table.on('tool(test-table-page)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
          ...
          if(layEvent === 'see'){ //审核
            layer.open({
              ...
              maxmin: true,
              area: ['90%', '100%'],
              content: "<?php echo e(url('/user/seestore?')); ?>"+e.store_id
            });
            ...
            $('.order').attr('lay-href',"<?php echo e(url('/user/tradelist?')); ?>"+e.store_id);

          }else if(layEvent === 'passway'){
            localStorage.setItem('store_store_name', e.store_name);
            $('.passway').attr('lay-href',"<?php echo e(url('/user/passway?')); ?>"+e.store_id);

          }else if(layEvent === 'branchshop'){
            
            $('.branchshop').attr('lay-href',"<?php echo e(url('/user/branchshop?id=')); ?>"+e.id+"&store_name="+e.store_name);

          }else if(layEvent === 'storecode'){
            $('.storename').html(e.store_name);
            $.post("<?php echo e(url('/api/user/store_pay_qr')); ?>",
            {
                token:token,
                store_id:e.store_id
            },
            ...
            $('.shouyin').attr('lay-href',"<?php echo e(url('/user/shouyin?store_id=')); ?>"+e.store_id+"&store_name="+e.store_name);
        ...
        $('.passway').click(function(){          
          $(this).attr('lay-href',"<?php echo e(url('/user/passway?')); ?>"+$('.js_store_id').val());
        });
        $('.branchshop').click(function(){
          $(this).attr('lay-href',"<?php echo e(url('/user/branchshop?id=')); ?>"+$('.js_id').val()+"&store_name="+$('.js_store_name').val());
        });
        $('.shouyin').click(function(){
          $('.shouyin').attr('lay-href',"<?php echo e(url('/user/shouyin?store_id=')); ?>"+$('.js_store_id').val()+"&store_name="+$('.js_store_name').val());
        });
        $('.merchantnumber').click(function(){
          $(this).attr('lay-href',"<?php echo e(url('/user/merchantnumber?')); ?>"+$('.js_store_id').val());
        })
        $('.merchantmanage').click(function(){
          $(this).attr('lay-href',"<?php echo e(url('/user/merchantmanage?')); ?>"+$('.js_store_id').val());
        })
        $('.fuyoumanage').click(function(){
          $(this).attr('lay-href',"<?php echo e(url('/user/fuyoumanage?')); ?>"+$('.js_store_id').val());
        })
        // *********end
        ...
        var active = {
          batchdel: function(){
            ...
            layer.confirm('确定删除吗？', function(index) {   
              ...
              $.post("<?php echo e(url('/api/user/del_store')); ?>",
              {
                  token:token,
                  store_id:arrs.join()                    
        ...
        var actives = {
          recover: function(){
            ...
            layer.confirm('确定恢复吗？', function(index) {   
              ...
              $.post("<?php echo e(url('/api/user/rec_store')); ?>",
              {
                  token:token,
                  store_id:arrs.join()                    
        ...
        var activet = {
          batchdelclose: function(){
            ...
            layer.confirm('确定关闭门店吗？', function(index) {   
              ...
              $.post("<?php echo e(url('/api/user/col_store')); ?>",
              {
                  token:token,
                  store_id:arrs.join()                    
        ...
        var activets = {
          batchdelopen: function(){
            ...
            layer.confirm('确定开启门店吗？', function(index) {   
              ...
              $.post("<?php echo e(url('/api/user/ope_store')); ?>",
              {
                  token:token,
                  store_id:arrs.join()                    
        ...
        var activett = {          
          transfer: function(){
            ...
            //   // $('.addstoretransfer').attr('lay-href',"<?php echo e(url('/user/addstoretransfer?store_id=')); ?>"+store_id+'&store_name='+store_name);
            // }
        ...
        $('.closestore').click(function(){
          ...
          $("input:radio[name='sex']:checked").each(function() { // 遍历name=standard选中的多选框的值
            ...
            console.log($(this).val());
            $.post("<?php echo e(url('/api/user/check_store')); ?>",
            {
                token:token,
        ...
        $('.addstore').click(function(){
          $(this).attr('lay-href',"<?php echo e(url('/user/editstore?0')); ?>");
        })
        $('.addstoretransfer').click(function(){
          $(this).attr('lay-href',"<?php echo e(url('/user/addstoretransfer?store_id=')); ?>"+$('.js_add_store_id').val()+'&store_name='+$('.js_add_store_name').val());
